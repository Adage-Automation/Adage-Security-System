...
2. **Primary, confirmed set up 2026-09-25**: [UptimeRobot](https://uptimerobot.com) (free tier), monitor `adage-security-system` pointed at `https://adage-security-system.onrender.com/api/health`, checking every 8 minutes — comfortably inside Render's 15-minute sleep window. Real fixed-interval scheduling (not best-effort like GitHub Actions), no inactivity-based disabling, and it emails on real downtime too, so it doubles as basic monitoring. This is what actually fixed a real observed symptom: the backend was still going to sleep in practice because the GitHub Actions pinger's real-world cadence (see below) wasn't tight enough on its own.

If the app goes quiet for 60+ days and the GitHub Actions pinger silently stops, the external uptime monitor keeps both services alive regardless — this is exactly why it's the primary pinger rather than relying on GitHub Actions alone.
...
See `backend/.env.example` for the full list. At minimum, production needs its own: `DATABASE_URL`, `SESSION_SECRET` (long, random, unique — never reuse the dev value), `AZURE_TENANT_ID`/`AZURE_CLIENT_ID`/`AZURE_CLIENT_SECRET`/`MAIL_FROM_ADDRESS`, `STORAGE_*`, `FRONTEND_URL`, `NODE_ENV=production` (this flips the session cookie to `secure`, which requires HTTPS), and optionally `SENTRY_DSN` (error tracking, 2026-09-25 — safe to leave unset). There is no security-email env var or Settings key — with multiple security units (2026-09-25), the CC on an emailed report is the sending account's own login email (`User.email`), set per account on the Users screen, not a single config value anywhere.
...
- [ ] Database backups are actually running — Supabase's free tier has no automated backups/PITR at the provider level, so this means `.github/workflows/db-backup.yml`'s repo secrets (`BACKUP_DATABASE_URL`, `STORAGE_*`) are set and a run has succeeded at least once (2026-09-25 — see `docs/decisions.md`)
- [ ] `SENTRY_DSN` is set if error tracking is wanted in this environment (optional, safe to skip — see `docs/decisions.md`)
