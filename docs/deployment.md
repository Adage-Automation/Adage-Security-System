...
**Two pingers currently in place**:
1. `.github/workflows/keep-alive.yml` — a GitHub Actions schedule (`*/10 * * * *`) that curls `/api/health`. Free, but has a real limitation: GitHub auto-disables scheduled workflows after 60 days with zero commits/pushes to the repo, silently, with no alert.
2. **Recommended primary**: an external uptime monitor (e.g. [UptimeRobot](https://uptimerobot.com), free tier — 5-minute interval) pointed at the same `/api/health` URL. No inactivity-based disabling like GitHub Actions has, and it emails on real downtime too, so it doubles as basic monitoring. See [decisions.md](./decisions.md#keep-alive-apihealth--a-scheduled-github-actions-ping) for the full reasoning.
